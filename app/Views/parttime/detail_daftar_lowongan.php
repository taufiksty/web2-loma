...
<div class="flex flex-col w-full lg:flex-row my-24">
  <div class="sticky top-20 px-14 h-full card rounded-box place-items-center">
    ...
    <p class="mt-3 font-bold text-xl"><?= $detail_parttime[0]['posisi']; ?></p>
    <p class="mt-3 font-semibold text-lg"><?= $detail_parttime[0]['nama_perusahaan']; ?></p>
    <p class="mt-3 text-lg"><?= $detail_parttime[0]['wilayah_penempatan']; ?></p>
    <p class="mt-3 text-lg">Ditayangkan pada <?= $interval; ?> hari yang lalu</p>
    <a href="<?= base_url(); ?>/Rekruter/lihatLamaranMasuk/Parttime/<?= $detail_parttime[0]['id']; ?>/<?= $detail_parttime[0]['id_rekruter']; ?>" class="btn btn-accent mt-9">Lihat Lamaran Masuk</a>
  </div>
